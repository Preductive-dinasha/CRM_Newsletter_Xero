...
class TestSummarisationTriggered:
    def test_summarise_triggered_above_threshold(self, app):
        with app.app_context():
            ...
            with patch.object(service, "_call_llm_summarise", return_value="Summary text"):
            ...
            assert "[Summary]" in call_args.kwargs.get("content", "")
    ...
    def test_summary_replaces_old_turns(self, app):
        with app.app_context():
            ...
            summary_msg.content = "[Summary] Condensed history"
            mock_repo.create.return_value = summary_msg

            service = SummarisationService(chat_repo=mock_repo)
            with patch.object(service, "_call_llm_summarise", return_value="Condensed history"):
