...
## Recent Changes
- 2026-06-03: Faithful HTML-mockup adoption ("adapt as it is") across auth + chat — Login/Signup cards rounded-2xl (16px), form labels 13px/700, fade-up entrance; Login gains "OR CONTINUE WITH" divider + Google/SSO outlined buttons (UI-only, no OAuth backend); chat input bar gains persistent header strip (agent context pill + "v2.4 Cognitive Model" + "AI PROCESSING READY") and a quick-chips row (Connect Data/Summarize Activity/Find Contacts → set agent); empty state gains two suggestion cards (Analyze last quarter sales / Identify at-risk accounts) wired to quick-send; glass-panel opacity 0.82; build passes; e2e login→chat verified
- 2026-06-03: Removed Mail/Lock icons from Login email/password inputs (clean inputs, px-4)
- 2026-06-03: Pai style-spec adoption — inputs bg white/50 + 8px radius + focus ring primary/20; primary buttons 8px radius, active scale 98%; "Forgot password" link 12px bold uppercase #0061a2; auth cards glassmorphism rgba(255,255,255,0.8) at 12px radius; logo badge 32px-radius squircle; chat input bar 22px container with #fcf2ec footer strip (+header strip on file attach); send button 40×40 at 12px radius, hover scale 105%; chips/badges full pill (skill chips #f0e6e0/#eae1db); build passes
